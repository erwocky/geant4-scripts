...
my ($start_run, $stop_run) = @ARGV;
$stop_run = ($stop_run >= $start_run) ? $stop_run : $start_run;
...
for (my $this_run=$start_run;$this_run<=$stop_run;$this_run++) {
          ...
          if ($num_thcross > 0) {
               ...
               if ($num_notborder > 0) {
                    ...
                    if ($num_localmax > 0) {
                         ...
                         my $tmp_img = zeros(long, $img->shape+2);
     ...
     #print "red maskcodes: ".($maskcode->index($indx_reds))."\n";
     #print "green maskcodes: ".($maskcode->index($indx_greens))."\n";
     #print "indx_reds: ".$indx_reds."\n";

     print "### Run $this_run: number of reds:   ".$indx_reds->nelem."\n";
     print "### Run $this_run: number of greens: ".$indx_greens->nelem."\n";
     print "### Run $this_run: number of cyans:  ".$indx_cyans->nelem."\n";
     print "### Run $this_run: 2-7 keV count rate: $flux_goodenergy cts/cm2/s/keV\n";

     my $hdr = { 'NFRAMES'=>$numtotframes, 'NBLOBS'=>$numtotblobs };

     # write out the event list
     print "### Writing event list for run $this_run.\n";
     my $outevtfile = "output/geant_events_${this_run}_evt.fits";
     ...
     my $outpixfile = "output/geant_events_${this_run}_pix.fits";
     $xdep += $x_offset;
     $ydep += $y_offset;
     ...
     my $outframefile = "output/geant_events_${this_run}_frames.fits";
     my $indx_sorted = $frame_frame->qsorti;
     ...
     my $outblobfile = "output/geant_events_${this_run}_blobs.fits";
     my $indx_sorted = $blob_blobid->qsorti;
