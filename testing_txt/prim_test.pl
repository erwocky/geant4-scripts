...
for (my $this_run=$start_run;$this_run<=$stop_run;$this_run++) {
    ...
    print FH "\n";
    foreach (@num_in_different_quadrants) {
        print FH "$_,";
    }
    print FH "\n";
    print_all($framedep);
    ...
    if ($test >= 2) {
        ...
        our $pixel_indx = which($framedep==$frame);
        $x = $xdep->index($pixel_indx)->sever;
        $y = $ydep->index($pixel_indx)->sever;
        $en = $endep->index($pixel_indx)->sever;
        our $pixptype = $ptypedep->index($pixel_indx)->sever;
        $pixprimid = $eiddep->index($pixel_indx)->sever;
        ...
        $xdim = $x->max+3;
        $ydim = $y->max+3;
        foreach (($x, $y, $en, $pixptype, $pixprimid, $this_blobid)) {
            print_all $_;
            print FH "\n";
        }
        print FH "$xdim,$ydim";
    ...
    if ($test >= 3) {
        ...
        foreach (($img,$ptypeimg,$primidimg,$frame_npix,$frame_npixmip,$frame_nprim)) {
            print "written\n";
            print_all $_;
            print FH "\n";
        }
...
my $p =  `python3 prim_test.py $test $start_run`;
print $p;
